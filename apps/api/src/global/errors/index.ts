...
export class CommonError extends Error {
  statusCode: keyof StatusMap = 'Internal Server Error'
  code?: number

  constructor(statusCode: keyof StatusMap, opt: { message: string; code?: number }) {
    const { message, code } = opt
    super(message)
  ...
  toResponse() {
    return Response.json(
      ...
      {
        status: STATUS_MAP[this.statusCode]
      }
    )
  }
}
